...
namespace MagicTower.Components.UIPanel {
    public class DataEditorPanel : MonoBehaviour {
        ...
        void Awake() {
            ...
            for (int i = 0; i < Game.Config.modals.Count; ++i) {
                // First TODO: modalList.Add(new Dropdown.OptionData(Game.Config.modals[i + 1].id + ". " + Game.Config.modals[i + 1].name, Modal.GetResourceBaseSprite(Game.Config.modals[i + 1].id)));
            }
        ...
        public void OnMapSave() {
            ...
            var path = ArmyAnt.Manager.IOManager.SaveToFile(System.Text.Encoding.UTF8.GetBytes(result), "MapData", number + ".json");
            Game.ShowTip("已保存成功！路径：" + path);
        }
        ...
        public void OnMapBackModalClick() {
            var panel = mapMakerPanel.transform.Find("SetPanel");
            var mapId = panel.transform.Find("MapId").GetComponent<Dropdown>().value;
            Game.ShowModalSelector(Game.Config.GetGameMap(mapId).backThing, (int id) => {
                Game.Map.ChangeBack(Game.Config.modals[id].prefabPath);
                Game.Config.GetGameMap(mapId).backThing = id;
                // First TODO: panel.transform.Find("btnBackModal").Find("Image").GetComponent<Image>().sprite = Modal.GetResourceBaseSprite(id);
        ...
        public void OnMapCurrentModalClick() {
            var panel = mapMakerPanel.transform.Find("SetPanel");
            var mapId = panel.transform.Find("MapId").GetComponent<Dropdown>().value;
            Game.ShowModalSelector(Game.Config.GetGameMap(mapId).blocks[posx][posy].thing, (int id) => {
                modalId = id;
                ...
                if (id == 0) {
                    ...
                    // First TODO: panel.transform.Find("btnCurrentModal").Find("Image").GetComponent<Image>().sprite = Modal.GetResourceBaseSprite(id);
        ...
        public void OnMapCurrentModalClear() {
            var panel = mapMakerPanel.transform.Find("SetPanel");
            var mapIndex = panel.transform.Find("MapId").GetComponent<Dropdown>().value;
            Game.Map.ChangeThingOnMap(0, posx, posy);
            var block = Game.Config.GetGameMap(mapIndex).blocks[posx][posy];
            block.thing = 0;
            Game.Config.GetGameMap(mapIndex).blocks[posx][posy] = block;
            panel.transform.Find("btnCurrentModal").Find("Image").GetComponent<Image>().sprite = null;
            panel.transform.Find("btnCurrentModal").Find("Text").GetComponent<Text>().text = Game.Config.StringInternational.GetValue("none");
        }

        public void OnMapClearAllBlocks() {
            var panel = mapMakerPanel.transform.Find("SetPanel");
            var mapIndex = panel.transform.Find("MapId").GetComponent<Dropdown>().value;
            var map = Game.Config.GetGameMap(mapIndex);
            for (var x = 1; x < Present.Map.View.MAP_BLOCK_LENGTH - 1; ++x) {
                for (var y = 1; y < Present.Map.View.MAP_BLOCK_LENGTH - 1; ++y) {
                    Game.Map.ChangeThingOnMap(0, x, y);
        ...
        void OnMapSelected() {
            ...
            // First TODO: panel.transform.Find("btnBackModal").Find("Image").GetComponent<Image>().sprite = Modal.GetResourceBaseSprite(Game.Config.GetGameMap(mapId - 1).backThing);
            ...
            Game.Map.ShowMap(mapId);
            panel.Find("SetWhenClick").GetComponent<Toggle>().isOn = false;
            OnMapClicked(0, 0);
        ...
        public void OnMapClicked(int posx, int posy) {
            ...
            var toggle = panel.transform.Find("SetWhenClick").GetComponent<Toggle>();
            if (toggle.isOn) {
                Game.Map.ChangeThingOnMap(modalId, posx, posy);
                var block = map.blocks[posx][posy];
                block.thing = modalId;
                map.blocks[posx][posy] = block;
            } else {
                modalId = map.blocks[posx][posy].thing;
            }
            if (modalId == 0) {
                panel.transform.Find("btnCurrentModal").Find("Image").GetComponent<Image>().sprite = null;
                panel.transform.Find("btnCurrentModal").Find("Text").GetComponent<Text>().text = "none";
            } else {
                // First TODO: panel.transform.Find("btnCurrentModal").Find("Image").GetComponent<Image>().sprite = Modal.GetResourceBaseSprite(modalId);
                panel.transform.Find("btnCurrentModal").Find("Text").GetComponent<Text>().text = Game.Config.StringInternational.GetValue(Game.Config.modals[modalId].name);
            }
        ...
        public void OnModalSelected(int index) {
            ...
            // First TODO: modalMakerPanel.transform.Find("ModalPrefabImage").GetComponent<Image>().sprite = Resources.Load<GameObject>(Modal.GetResourcePath(index)).GetComponent<SpriteRenderer>().sprite;
        ...
        public void OnPrefabSelected(RectTransform sender) {
            ...
            // First TODO: modalMakerPanel.transform.Find("CurrentPrefab").GetComponent<Image>().sprite = Resources.Load<GameObject>(Modal.GetResourcePath(prefabPath)).GetComponent<SpriteRenderer>().sprite;
        ...
        public void OnReset(int part) {
            switch (part) {
                case 0:
                    if (ArmyAnt.Manager.IOManager.RemoveFolder("MapData")) {
                        Game.ShowTip("Clean map json files successful");
                    } else {
                        Game.ShowTip("Clean map json files failed");
                    }
                    break;
                case 1:
                    OnModalSelected(0);
        ...
        public void OnSaveString() {
            ...
            if (key != newKey && Game.Config.strings.ContainsKey(newKey)) {
                Game.ShowTip("保存失败, key值重复!");
                return;
            }
            if (newKey == "") {
                Game.ShowTip("保存失败, key不能为空!");
                return;
            }
            ...
            if (stringList.Count < 1) {
                Game.ShowTip("保存失败, 至少需要有一种语言拥有字符串值");
                return;
            }
            if (key == newKey) {
