...
1. 事件结构已优化, 基本通过测试
2. 地图编辑器的 bug 已修复, 现在可以正常编辑地图了
3. 获取基本资源型道具的事件已完成, 需要进一步测试（在2楼摆放更多物品进行测试）
4. 跨地图传送事件已通过测试，但是有一个 bug (见下)
5. 怪物数据编辑器需要进行便利性优化, 这是接下来要进行的工作
6. Bug: 重要级，跨地图第一次传送时，由于载入延迟，导致角色传送后多走了一步
7. Bug: 一般级，退出地图编辑器然后进入游戏时，报错，无法正常游戏
8. Bug: 轻微级，开门时，仍然可以继续触发开门事件（会继续扣钥匙并重复播放音效），应加入一个开门 loading 状态来解决此问题
...
## 2019-4-3-Wednesday
1. 修复了编辑器的一些小问题, 添加了很方便的批量点击放置物品的功能, 添加了一键清除地图上所有物品的功能
2. 发现一个编辑器bug: 播放音乐时, 切换地图导致音乐停止后, 再次播放相同的音乐无法生效, 直到播放另一个音乐
3. 完成了双选对话框, 完成了游戏主菜单的界面部分, 需要添加按钮回调逻辑和按钮字样的国际化字符串
4. 正在制作 setting 框

## 2019-4-9-Tuesday
1. 需要填充 setting 框内容
2. 需要制作 SaveLoadGame 框

## 2019-4-10-Wednesday
1. Setting 框制作完成, 相应数据也已实装, 但需要应用到具体位置
2. 所有新做的框需要适配国际化字符串
3. 需要测试所有新做对话框的适配
...
## 2019-4-17-Wednesday
1. 仍在制作存读档对话框, 可能需要为了显示而添加更多存档内容, 例如时间

## 2019-6-4-Tuesday
1. 最近花了很多天时间对项目进行大规模重构, 重构后出现了一系列问题, 需要进一步检查和整理. 由于改动量过大, 所以先提交一次, 记录下问题:
   + ~~Sprite 记录到了 Loading 节点 Prefab 中, 但是蓝色门(或者有可能还有其他门)的资源错了~~
   + ~~Map 在转换时仍然会出现缺少地图块的问题, 很多地图块(尤其是墙)刚加载出来就被移除, 这可能是 Modal 方面的问题, 需要进一步检查~~
   + ~~GameOver 资源中间的黑块有点穿帮~~
   + ~~退回到主界面再次开始新游戏时出现问题, 需要对上一局游戏进行清理(包括数据和场景)~~
   + ~~ChoiceDlg 报错于 Game.GetNumberData~~
2. 文件操作已重新设计, 改成了同步阻塞方式
3. 只保留主场景, 所有内容切分成 UI-Panel_prefab 和 Sprite-Prefab, 由 Loading-Object-Prefab 负责加载, 目前是立即加载的, **需要添加延迟加载的方案**
4. 需要对重构前已完成的功能进行全面测试以查找因重构造成或暴露的问题
5. 所有对话框现在成了即用即销毁模式, **需要研究是否有必要改为预加载为单例**
6. 资源已进行了移动, 现在所有用到的资源(除旧的 Resource/Prefab 引用的 player 资源外), 都在 Assets/Image/Used 中, **未来考虑进一步筛选有用资源, 将无用资源移除**
7. Unity 引擎版本稳定在 2018.4 的 LTS版本, 在发布前预计会稳定使用此版本不会升级至 2019

## 2019-6-5-Wednesday
1. 基本完成了对重构之后问题的处理, 重修了 GameOver 的资源图, 解决了几个暴露出来的 bug, 目前整个项目的结构比之前清晰多了.
2. 接下来做完存读档功能后, 会做 Readme 对话框, 然后对UI进行一波资源替换, 替换时会对所有用到的资源进行集中整理, 无用的UI资源将被清理出项目文件夹

## 2019-6-6-Thursday
1. 完成了存读档功能, 并将存读档的 json 数据结构移动到 Constant.cs 里

## 2019-6-15-Saturday
1. 完成了用于长文本提示框的 Info Dialog
2. 完成了资源替换, 删除了无用的散图标和UI资源, 调整了一些界面的适配参数
3. 设置项添加了关于方向杆和自动寻路的开关
4. 有如下关键事项需要接下来完成:
    + ~~存读档框需要添加一个能够删除存档的按钮~~([2019年6月18日](#2019-6-18-Tuesday)解决)
    + 需要整理战斗框的外观, 使其更美观, 尤其是战斗结算条的位置, 以及更多提示 (双方技能, 某些按钮如撤退)
    + ~~需要实现虚拟方向杆~~([2019年6月19日](#2019-6-19-Wednesday)完成)
    + 设置界面需要继续添加必要的设置项, 并实现所有的设置项, ~~需要仔细检查并修正设置界面所有元素的适配对齐, 尤其是字体大小的适配~~([2019年7月3日](#2019-6-19-Wednesday)完成)
    + **需要完成 Readme 的文本内容**
    + **需要全面整理数据编辑器的功能**
    + ~~需要将 player 和所有 json 全都加入到 Global Loading~~([2019年6月20日](#2019-6-20-Thursday)完成了player的加入, json决定不加入)
    + ~~设置延迟加载策略, 添加延迟加载时的闪屏~~([2019年7月4日](#2019-7-4-Thursday)完成)
    + ~~需要添加一个设置项, 指示是否将所有 Panel 加入到 Scene 的单例中, 如是, 则加入, 节省加载时间, 否则节省运行时内存~~ (区别并不算大, 暂不实行)
    + 开发一些特殊道具的功能, 例如楼层飞翔器, 怪物数值查看器, 对话内容记录器, 包括相关UI和数据
    + 开发怪物的 "特殊技能" 功能, 例如双倍暴击伤害、魔法伤害、魔法抵抗、暴击反击、暴击闪避、闪避反击、连击、暴击连击、坚壁(受到伤害不超过指定值)、镜面(攻击力总与玩家等同)、毒刃(攻击附加可叠加的中毒状态, 中毒后每次攻击自身损失生命, 战斗结束后消除)、伪装(怪物数值查看器查不到准确数值)、背水（血量越低攻击力越高）, 包括相关UI资源
    + 在 Editor 上添加一系列便利性功能, 将Unity的功能充分利用起来
5. 完成上述关键事项后, 另有如下事项需要后续完成:
    + **需要设计应用图标**和 ~~ArmyAnt 组织图标, 这两者还需要全屏大图标以用于闪屏展示~~([2019年7月30日](#2019-7-30-Tuesday)完成组织初版简易图标)
    + **需要制作 Miss 和跳数字**
    + **需要制作一个 net-loading 标识, 以应对将来可能需要的 HTTP 网络功能**
    + 优化寻路算法, 目前该算法问题很大, 考虑改用递归算法, 但递归算法应严格注意空间复杂度
    + 完成 Analytics 的功能及埋点
    + 设计复活机制, 嵌入广告及付费功能
    + **对前十层设计Demo**
    + 完成 steamwork api 的接入
    + **考虑将闪屏 SplashPanel 写得更加通用化**
6. [2019年8月7日](#2019-8-7-Wednesday)备注: 上述第4和第5点中, 加粗项正在进行, 非加粗的未完成项, 将在后续版本中通过规范的文档形式记录, 不再在此处记录

## 2019-6-18-Tuesday
1. 完成了对存档功能及存档框的翻新, 现在, 整个界面更好看了, 操作也更方便, 并且增加了截图, 以及删除按钮
2. SceneView 移到了 Main Camera 上, 以便能够调用 OnPostRender
3. 将一些子界面用到 TipBar 的地方改成了 AlertDialog. 今后, 将尽量避免关卡之外的通知使用 TipBar
4. 增加了一个单选列表 SelectListView, 处理 EditorPanel 时将用之替代 ModalSelector 中的列表

## 2019-6-19-Wednesday
1. 重新整理了 InputManager, 改为事件驱动模式, 回调函数分散在各自的功能块里, 并添加Call功能以兼容虚拟方向杆
2. 添加了一个基本的虚拟方向杆, ~~还需要加入一个隐藏按钮~~[2019年7月2日](#2019-7-2-Tuesday).

## 2019-6-20-Thursday
1. 将国际化字符串的配置拆分出来, 只读取当前语言的字符串.
2. 将 TextAsset 直接挂载到 prefab上无法做到动态加载, 因此保留了配置文件的 Resource 读取方式
3. 移除了刚进入游戏就加载全部地图数据的方式
4. 完成了 Readme 的内容, 完善了 Readme 的功能, 现在可以加入很多帮助信息
5. 修复了一个 bug, 曾导致 StartPanel 修改语言时, 主界面 PlayerPanel 信息栏不会生效
6. 修复了 SelectListView 的一个无法设置初始选择的bug

## 2019-7-2-Tuesday
1. 完成了虚拟方向杆的设置项启用开关和显示/隐藏按钮, 完成了点击自动寻路功能的设置项启用开关
2. 检查了一下适配相关的情况, 处理了一下适配问题. canvas 的适配模式改成了 Expand. 目前适配方面仍存在一些问题, 以下是记录一些零散的适配问题情况:
    + 主界面几个图标和文字的适配需要优化
    + 地图显示, 以及鼠标所点到的地图位置检测, 在其他分辨率下是错误的

## 2019-7-3-Wednesday
1. 对地图块的适配进行了统一调整, Sprite不再设置特别的缩放(均设置为1), 而是设置摄像机的缩放为3, 将UI的计算像素固定为宽1280. 这个基本可以满足所有分辨率比例了(从16:9到5:4), 有两个问题, 目前暂且忽略:
    + 屏幕高度低于480的话会有明显的模糊, 这个不做处理, 因为本游戏地图本来就非常大, 不需要适配这种极老的小分辨率屏幕, 但是后续需要把过小的相关分辨率选项去除
    + 长宽比过低会导致地图区域稍微有点小, 边缘会被半个墙遮挡, 这是由于camera的scale计算不严谨导致的. 由于按照目前设计所有地图的边缘都会围一圈实墙, 故暂不做处理
2. 修复了寻路的bug. 经查明, 该bug与地图计算无关, 而是因为 StartAutoStep 与 StopAutoStep 被在两个不同的地方加入了监听, 导致在一次点击中先后调用, 从而不能寻路

## 2019-7-4-Thursday
1. 完成了闪屏机制, 目前缺闪屏图标, 以及淡入淡出机制. 但是发现场景载入时延迟较高, 此时尚未出现闪屏, ~~有待优化~~([2019年7月30日](#2019-7-30-Tuesday)完成).

## 2019-7-30-Tuesday
1. 将 LoadingObject 的 prefab 改为从 Resource 加载, 从而解决了载入前卡顿的问题, 同时优化了闪屏显示, 完善了淡入淡出效果
2. 制作了一个简易的 ArmyAnt logo 画面
3. 接下来先集中精力整理编辑器功能了

## 2019-7-31-Wednesday
1. 重新开始构建编辑器, 共有12个子面板需要构建, 目前完成了第一个: 音效设置面板
2. 修复了关于 AudioManager 的一个 bug, 该 bug 曾导致, 当音乐被 StopMusic 暂停后, 不改变 id 就无法恢复播放

## 2019-8-2-Friday
1. 为了美化样式, 添加了微软雅黑字体至资源, 除设置界面和编辑器相关的几个界面外, 所有界面字体已替换成该字体, 后续需要把剩余未替换字体的界面也替换掉
2. 取消了绝大部分文本的字体自适应, 避免窗口放到很大时, 字体过大造成比较难看的感觉
3. 修复了一个覆盖进度必失败的 bug, 该 bug 原因是, 逻辑层中, 当文件夹已存在时, MkdirIfNotExist 会返回 false
4. 修复了一个 bug, 该 bug 曾导致主界面读取进度的窗口被隐藏, bug 原因是引入截屏功能时, 自动隐藏了该窗口, 但重新显示窗口却要求在主界面之外, 解决方法是主界面存在时不进行截屏
5. 修复了一个 bug, 该 bug 曾导致幕布和虚拟摇杆被放大120倍因而找不到, bug 原因是闪屏代码加载这两个 prefab 时, 没有添加 SetParent 的第二个参数为 false
6. 正在制作 Modal 配置界面, 界面摆放基本完成, 将开始添加逻辑

## 2019-8-6-Tuesday
1. 将微软雅黑字体应用到所有地方, 调整了设置界面的字体大小适配
2. Modal 界面的~~名字编辑和~~**事件数据编辑**需要另外的框体支持, 新添加Modal功能暂时不做, 除此以外其他功能都已经完成
3. 优化了SelectListView, 覆写了多个虚函数以处理选择项因其他操作(删除项, 添加项等)发生变动时的情况, 并同时修复了 Audio 设置界面因此导致的一些bug
4. 已经向 Unity Technology 上报了 Dropdown 的 list canvas 层级 bug 问题, 如果在未来被修复, 可以删除 ListViewBugFix （据称在2019版本中得到了修复， 如果2020年发布可以考虑升级Unity并且删除该脚本）
5. Monster 界面的 special **特殊能力设置功能**有待将来完善, 除此以外其他编辑器功能均已完成
6. Player 界面和 Monster 界面高度相似, 但以后可能差异化, 目前 Player 编辑器已完成

## 2019-8-7-Wednesday
1. 完成了 weapon 编辑器的功能
2. 完成了 language 多语言参数编辑器
3. 完成了 stringMakerPanel, 并优化了运行时多语言字符串类的功能, 使之可以进行编辑器改动结果的测试
4. 为 modalmaker 和 weapon 编辑器补上了名字的字符串选择功能, 现在考虑到便利性, ~~应在以下界面也加入名字修改的功能: Monster, Player~~([2020年1月2日](#2020-1-2-Thursday)完成)
5. [2019年6月15日](#2019-6-15-Saturday)所列的一系列功能中, 已将接下来需要完成的功能加粗, 其余未完成的非加粗项, 将在后续版本中通过规范的文档形式记录, 不再在此文档中记录

## 2019-8-8-Thursday
1. 除了 eventData 编辑以及 Modal 选择, 完成了 chat 编辑器
2. Modal 选择器将来会重做, 但在 chat 以及 choice 界面使用频率会比较低
3. Choice 编辑器的界面排布完成, 只差代码.
4. 发现 Choice 选择框的组件过于老旧, ~~需要将ChoiceDlg的列表组件替换为 SelectListView, 并更新代码~~([2020年1月2日](#2020-1-2-Thursday)完成)

## 2019-12-31-Tuesday
1. 完成 choice 编辑器的代码
2. Modal选择框暂时弃用, 先用Modalmaker, ~~以后考虑优化后再用ModalSelector~~([2020年1月2日](#2020-1-2-Thursday)完成)
3. 修复了 string 编辑器的一个 bug, 该 bug 曾导致没有编辑数据的新机器上, string 编辑器无法使用, 产生原因是没有处理无缓存文件的情况, 详见本日 git 文件修改情况
4. 现在完成了编辑器的大体功能, 接下来需要做几个工作彻底完成编辑器:
    + ~~将一些特殊设置项加入到 miscellaneous 编辑器~~
    + ~~完成 info 编辑器的功能~~([2020年1月3日](#2020-1-3-Friday)完成)
    + 完成 map editor 的功能
    + 增加并完成 event 编辑器, 然后把它嵌入到其他编辑器的选择功能中, 补全所有编辑器暂时缺失的功能
    + 反复自测编辑器功能, 修复 bug 并优化不方便的点
    + 完善功能, 例如一些编辑器添加增删项目
5. 项目编辑器升级到了 2018.4.13f1
6. 新的一年要到了, 希望本项目的开发不会受到这几个月拖时间的影响, 能在2020年顺利完成第一版, 加油!

## 2020-1-2-Thursday
1. 重做了 ModalSelector, 用最新的写法优化了选择, 然后去掉了 ModalMaker 中暂时顶替相关功能的代码
2. 优化了 ModalImage, 现在可以显示正常大小的大型物件

## 2020-1-3-Friday
1. 优化了 Modal 的代码, 使开门动画和 hitter 动画更加平滑
2. 完成了 info 编辑器的功能, 但由于Game中对info的数据存储在固定数组中, 暂时无法实现增删

## 2020-1-13-Monday
1. 在 miscellaneous 中添加了正常的逻辑, 由于目前设置项不多所以暂时还没什么内容
2. 正在制作event data editor, 已完成UI排布, 并在注释中写明了每种事件的数据结构

## 2020-1-14-Tuesday
1. 鉴于文档到这里比较杂乱了, 这里再次整理一下, 前面的就不用找了.
2. 制作了一个 loading 标识, 以应对将来可能需要的 HTTP 网络功能, 或者其他需要延时的地方
3. 用普通的文本制作了 Miss 和跳数字, 效果还可以
4. 需要接下来完成的工作:
     + 完成并优化编辑器的 event data editor
     + 完成编辑器的 map editor 的功能
     + 反复自测编辑器功能, 修复 bug 并优化不方便的点, 完善功能, 在可能的地方添加增删项目的功能
     + 使用制作完善的编辑器编辑前10层的demo
     + 需要设计应用图标
5. 需要在上一步完成后考虑优化或修正的地方:
     + 考虑将闪屏 SplashPanel 写得更加通用化
     + 优化 hitter 的表现
     + 改善 Readme 的文本内容
     + 优化设置界面的设置项和表现适配
     + 优化各个弹框的UI和文字的适配情况, 检查各种分辨率下的美观情况
     + 优化寻路算法和寻路的表现
     + 重构数据代码的结构, 将现有的静态数据, 配置数据, 临时数据和存储数据划分得更加清晰
6. 需要规划到新文档中的功能在此记录备忘:
     + 完成 Analytics 的功能及埋点
     + 设计复活机制, 嵌入广告及付费功能
     + 完成 steamwork api 的接入
     + 开发一些特殊道具的功能, 例如楼层飞翔器, 怪物数值查看器, 对话内容记录器, 包括相关UI和数据
     + 开发怪物和玩家的 "特殊技能" 功能, 例如双倍暴击伤害、魔法伤害、魔法抵抗、暴击反击、暴击闪避、闪避反击、连击、暴击连击、坚壁(受到伤害不超过指定值)、镜面(攻击力总与玩家等同)、毒刃(攻击附加可叠加的中毒状态, 中毒后每次攻击自身损失生命, 战斗结束后消除)、伪装(怪物数值查看器查不到准确数值)、背水（血量越低攻击力越高）, 包括相关UI资源
     + 在 Editor 上添加一系列便利性功能, 将Unity的功能充分利用起来. 可能的话, 把编辑器挪到Editor中
